package com.ilhom.equalizer.view.customView
...
class AnalogController : View {
    var midx = 0f
    var midy = 0f
    var textPaint: Paint by notNull()
    var circlePaint: Paint by notNull()
    var circlePaint2: Paint by notNull()
    var linePaint: Paint by notNull()
    var angle: String by notNull()
    var currdeg = 0f
    var deg = 3f
    var downdeg = 0f
    var progressColor = 0
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val tmp2 = deg / 24
        val x1 = midx + (radius * (2f / 5) * Math.sin(2 * Math.PI * (1.0 - tmp2))).toFloat()
        val y1 = midy + (radius * (2f / 5) * Math.cos(2 * Math.PI * (1.0 - tmp2))).toFloat()
        val x2 = midx + (radius * (3f / 5) * Math.sin(2 * Math.PI * (1.0 - tmp2))).toFloat()
        val y2 = midy + (radius * (3f / 5) * Math.cos(2 * Math.PI * (1.0 - tmp2))).toFloat()
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        ...
        if (e.action == MotionEvent.ACTION_DOWN) {
            ...
            downdeg = floor(x = (downdeg / 15).toDouble()).toFloat()
            return true
        }
